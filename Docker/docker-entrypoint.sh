...
# --- FONCTION DEDIEE A LA CONVERSION PPK ---
convert_keys_to_ppk() {
    echo "--- ? Démarrage de la conversion PPK pour PuTTY... ---"
    
    KEY_DIR="/data/private_keys"
    
    find "$KEY_DIR" -type f -name "*_ssh_key" | while read -r KEY_PATH; do
        PPK_PATH="${KEY_PATH}.ppk"
        if [ ! -f "$PPK_PATH" ]; then
            echo "Conversion de: $(basename "$KEY_PATH") en PPK..."
            # Correction PuTTY : -O private
            puttygen "$KEY_PATH" -o "$PPK_PATH" -O private
            chmod 600 "$PPK_PATH"
        else
            echo "Le fichier PPK pour $(basename "$KEY_PATH") existe déjà. Ignoré."
        fi
    done
    echo "--- Conversion PPK terminée. ---"
}
...
# --- RESTAURATION DES BINAIRES ---
if [ ! -f "/data/bin/ffmpeg" ]; then
    echo "[INIT] /data/bin est vide. Restauration depuis le coffre-fort..."
    mkdir -p /data/bin
...
while IFS=: read -u 3 -r TARGET_USER TARGET_PASS TARGET_PUID TARGET_PGID || [ -n "$TARGET_USER" ]; do
    ...
    # --- COPIE DES SCRIPTS (Optimisé & Safe) ---
    if [ -d "$USER_SKEL_DIR" ]; then
        # copie récursive (fichiers + dossiers + fichiers cachés)
        cp -r "$USER_SKEL_DIR"/. "$TARGET_SCRIPTS_DIR"/

        # rend seulement les scripts (.sh et .py) exécutables
        find "$TARGET_SCRIPTS_DIR" -type f \( -iname "*.sh" -o -iname "*.py" \) -exec chmod +x {} \;
    fi
    
    # --- CREATION DOSSIER VIDEOS ---
    mkdir -p "$TARGET_HOME_DIR/Videos"
